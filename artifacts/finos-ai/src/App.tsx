    ...
    <div className="flex min-h-[100dvh] min-w-0 flex-1 flex-col md:pl-[248px]">
      <header className="sticky top-0 z-20 flex h-[73px] items-center gap-3 border-b border-[#172c40] bg-[#081522]/90 px-4 backdrop-blur-xl md:px-8">
        ...
        <div className="hidden h-7 w-px bg-[#203447] sm:block" />
         <div className="relative"><button onClick={()=>setUserMenuOpen(!userMenuOpen)} className="flex items-center gap-2 rounded-lg p-1.5 text-left hover:bg-[#142a3b]" data-testid="button-user-menu"><div className="grid h-7 w-7 place-items-center rounded-full bg-[#cb9eeb] text-[10px] font-bold text-[#182033]">{platform.user.initials}</div><span className="hidden text-[12px] font-semibold text-[#cddbe6] lg:block">{platform.user.name}</span><ChevronDown size={13} className="hidden text-[#718ba1] lg:block"/></button>{userMenuOpen&&<div className="absolute right-0 top-11 z-30 w-48 rounded-xl border border-[#29465d] bg-[#0d1e30] p-1.5 shadow-2xl"><Link href="/profile" onClick={()=>setUserMenuOpen(false)} className="flex items-center gap-2 rounded-lg px-3 py-2 text-[11px] text-[#c7d8e1] hover:bg-[#173449]"><UserRound size={14}/> My profile</Link><Link href="/notifications" onClick={()=>setUserMenuOpen(false)} className="flex items-center gap-2 rounded-lg px-3 py-2 text-[11px] text-[#c7d8e1] hover:bg-[#173449]"><Bell size={14}/> Notifications {platform.unreadNotifications > 0 && <span className="ml-auto text-[#ff9b90]">{platform.unreadNotifications}</span>}</Link><button onClick={()=>{platform.toggleTheme();setUserMenuOpen(false)}} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-[11px] text-[#c7d8e1] hover:bg-[#173449]">{platform.theme==='dark'?<Sun size={14}/>:<Moon size={14}/>} Use {platform.theme==='dark'?'light':'dark'} mode</button><button onClick={()=>{go('/settings');setUserMenuOpen(false)}} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-[11px] text-[#c7d8e1] hover:bg-[#173449]"><Settings size={14}/> Settings</button></div>}</div>
      </header>
       {notificationsOpen&&<div className="absolute right-5 top-[63px] z-30 w-[310px] rounded-xl border border-[#29465d] bg-[#0d1e30] p-4 shadow-2xl" data-testid="panel-notifications"><div className="mb-3 flex items-center justify-between"><span className="font-semibold text-[#e6f1f6]">Notifications</span><Link href="/notifications" onClick={()=>setNotificationsOpen(false)} className="text-[10px] text-[#58d5e6]">View all</Link></div>{platform.notifications.slice(0,3).map((item)=><button key={item.id} onClick={()=>{platform.markNotificationRead(item.id);setNotificationsOpen(false);setLocation('/notifications')}} className="mb-3 block w-full border-l-2 border-[#58d5e6] py-1 pl-3 text-left last:mb-0"><div className="text-[12px] text-[#d9e9ef]">{item.title}</div><div className="mt-1 text-[10px] text-[#758da2]">{item.time}</div></button>)}</div>}
...
function Dashboard() {
  ...
  return <div className="mx-auto max-w-[1450px]"><SectionHeader eyebrow={`${localDateTime} / ${tenant.name}`} title={`Good morning, ${preferences.workspaceName}.`} description={`Here’s the operating picture for ${tenant.name}. Your AI team has been quietly moving things forward.`} action={<div className="flex items-center gap-2"><button onClick={()=>setBriefOpen(true)} className="btn-quiet flex items-center gap-2 rounded-lg px-3 py-2 text-xs" data-testid="button-morning-brief"><Sparkles size={14} className="text-[#58d5e6]"/> Morning brief</button><button onClick={()=>{setSaved(!saved);toast.success(saved?'Brief unsaved':'Brief saved to reports')}} className="btn-primary flex items-center gap-2 rounded-lg px-3 py-2 text-xs" data-testid="button-save-view">{saved?<Check size={14}/>:<Plus size={14}/>} {saved?'Saved':'Save view'}</button></div>}/><div className="mb-6 flex items-center gap-3 rounded-xl border border-[#25465d] bg-[#0c2130] px-4 py-3 text-[12px]"><span className="live-dot h-2 w-2 rounded-full bg-[#58d5e6]"/><span className="text-[#c3dbe3]">All systems nominal</span><span className="text-[#718da1]">•</span><span className="text-[#7e9aad]">Payments are settling within normal bounds</span><button onClick={()=>setLive(!live)} className="ml-auto text-[#58d5e6]" data-testid="button-toggle-live">{live?'Live':'Paused'}</button></div>
    ...
    <div className="mt-5 grid gap-5 lg:grid-cols-[1.2fr_.8fr]"><div className="panel fade-up delay-3 overflow-hidden"><div className="flex items-center justify-between border-b border-[#1b3448] px-5 py-4"><div><div className="kicker mb-1">Latest activity</div><div className="text-sm font-semibold text-[#dcebf1]">Recent transactions</div></div><Link href="/transactions" className="text-[11px] text-[#58d5e6]" data-testid="link-view-transactions">View all <ChevronRight size={12} className="inline"/></Link></div><div className="scrollbar overflow-x-auto"><table className="w-full min-w-[600px] text-left"><thead><tr className="border-b border-[#1b3448] text-[10px] uppercase tracking-[.12em] text-[#668096]"><th className="px-5 py-3 font-medium">Transaction</th><th className="px-3 py-3 font-medium">Merchant</th><th className="px-3 py-3 font-medium">Amount</th><th className="px-3 py-3 font-medium">Status</th><th className="px-5 py-3 text-right font-medium">Time</th></tr></thead><tbody>{tenantTransactions.slice(0,5).map((tx)=><tr className="table-row border-b border-[#142b3e] text-[12px]" key={tx.id}><td className="px-5 py-3.5"><span className="mono text-[10px] text-[#a6c2d0]">{tx.id}</span></td><td className="px-3 py-3.5 text-[#d7e5eb]">{tx.merchant}</td><td className="mono px-3 py-3.5 text-[#dcebf1]">${tx.amount.toLocaleString()}</td><td className="px-3 py-3.5"><Status>{tx.status}</Status></td><td className="px-5 py-3.5 text-right text-[#71899d]">{tx.time}</td></tr>)}</tbody></table></div></div><div className="panel fade-up delay-4 p-5"><div className="kicker mb-1">Merchant health</div><div className="mb-5 flex items-end justify-between"><div className="text-sm font-semibold text-[#dcebf1]">Portfolio pulse</div><Link href="/merchants" className="text-[11px] text-[#58d5e6]" data-testid="link-merchant-pulse">Details <ChevronRight size={12} className="inline"/></Link></div><div className="mb-4 flex items-center gap-5"><div className="relative h-24 w-24 shrink-0 rounded-full" style={{background:`conic-gradient(#52d4b0 0 ${Math.round(tenantMerchants.filter((item) => item.health === 'Healthy').length / Math.max(tenantMerchants.length, 1) * 100)}%, #f2c66a 0 91%, #ff8576 91% 100%)`}}><div className="absolute inset-[9px] grid place-items-center rounded-full bg-[#0c1b2c]"><div className="display-font text-xl font-semibold text-[#e5f3f5]">{tenantMerchants.length ? Math.round(tenantMerchants.filter((item) => item.health === 'Healthy').length / tenantMerchants.length * 100) : 0}</div></div></div><div className="space-y-2 text-[11px] text-[#8298a9]"><div className="flex items-center gap-2"><i className="h-2 w-2 rounded-full bg-[#52d4b0]"/>Healthy <b className="text-[#cae5e2]">{tenantMerchants.filter((item) => item.health === 'Healthy').length}</b></div><div className="flex items-center gap-2"><i className="h-2 w-2 rounded-full bg-[#f2c66a]"/>Review <b className="text-[#cae5e2]">{tenantMerchants.filter((item) => item.health === 'Review').length}</b></div><div className="flex items-center gap-2"><i className="h-2 w-2 rounded-full bg-[#ff8576]"/>At risk <b className="text-[#cae5e2]">{tenantMerchants.filter((item) => item.health === 'At risk').length}</b></div></div></div><div className="signal-line my-4"/><div className="flex items-center justify-between text-[11px]"><span className="text-[#7890a3]">Portfolio volume trend</span><span className="text-[#52d4b0]">+12.2%</span></div><div className="mt-2 h-8"><Sparkline values={[39,45,40,53,47,58,60,55,72,67,78,85]} color="#52d4b0" fill/></div></div></div>
...
function EmployeeForm({ employee, onClose }: { employee?: Employee; onClose: () => void }) {
  ...
  return <Modal title={employee ? `Edit ${employee.name}` : 'Add AI employee'} onClose={onClose}>
    <div className="scrollbar max-h-[78vh] space-y-4 overflow-y-auto pr-1">
      ...
      <div><span className="kicker mb-2 block">Permissions</span><div className="grid gap-2 sm:grid-cols-2">{builderPermissions.map((permission) => <label key={permission} className="flex items-center gap-2 rounded-lg border border-[#1b3549] bg-[#091725] px-3 py-2 text-[11px] text-[#a9bfcc]"><input type="checkbox" checked={draft.permissions.includes(permission)} onChange={(e) => setDraft({ ...draft, permissions: e.target.checked ? [...draft.permissions, permission] : draft.permissions.filter((item) => item !== permission) })} className="accent-[#58d5e6]" data-testid={`checkbox-permission-${permission.replace(':', '-')}`}/><span>{permission}</span></label>)}</div></div>
...
function EmployeeBuilderPage() {
  ...
  return <div className="mx-auto max-w-[900px]">
    ...
    <div className="panel p-5 md:p-6">
      <div className="mb-5 flex items-start gap-3 rounded-xl border border-[#25465d] bg-[#0c2130] p-4"><div className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-[#173b48] text-[#58d5e6]"><Bot size={18}/></div><div><div className="text-sm font-semibold text-[#dcebf0]">Employee configuration</div><p className="mt-1 text-[11px] leading-5 text-[#7892a5]">This employee will be visible only inside the active company workspace and will persist there for future sessions.</p></div></div>
    ...
    <div className="mb-7 flex flex-col justify-between gap-5 sm:flex-row sm:items-start">
      <div className="flex min-w-0 items-start gap-4">
        ...
        <div className="min-w-0">
          ...
          <h1 className="display-font truncate text-[30px] font-semibold tracking-[-.04em] text-[#edf7fa]">{liveEmployee.name}</h1>
          <div className="mt-2 flex flex-wrap items-center gap-2 text-[12px] text-[#8299ab]">
            <span>{liveEmployee.role}</span><span className="text-[#3c566d]">•</span><span>{liveEmployee.department}</span>
            <span className="text-[#3c566d]">•</span><span className="flex items-center gap-1.5" style={{ color: liveEmployee.color }}><span className={`live-dot h-1.5 w-1.5 rounded-full ${liveEmployee.active ? '' : 'opacity-30'}`} style={{ background: liveEmployee.color }}/>{status}</span>
          </div>
        </div>
      </div>
      <div className="flex flex-wrap gap-2">
      ...
      <div className="space-y-5">
        <div className="panel p-5">
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-1">
            <div><div className="text-[10px] uppercase tracking-[.12em] text-[#668096]">Job title</div><div className="mt-1 text-sm text-[#dcebf0]">{liveEmployee.role}</div></div>
            <div><div className="text-[10px] uppercase tracking-[.12em] text-[#668096]">Department</div><div className="mt-1 text-sm text-[#dcebf0]">{liveEmployee.department}</div></div>
            <div><div className="text-[10px] uppercase tracking-[.12em] text-[#668096]">Manager</div><div className="mt-1 text-sm text-[#dcebf0]">{liveEmployee.manager}</div></div>
            <div><div className="text-[10px] uppercase tracking-[.12em] text-[#668096]">Status</div><div className="mt-2"><span className={`status-pill ${liveEmployee.active ? 'border-[#2f8c7b] bg-[#12352f] text-[#6fe0bd]' : 'border-[#52627b] bg-[#1b293b] text-[#9db2ca]'}`}>{status}</span></div></div>
          </div>
        </div>
        <div className="panel p-5">
        ...
        <div className="panel p-5">
          ...
          <div className="mt-4 grid gap-2">{liveEmployee.permissions.map((permission) => <div key={permission} className="flex items-center gap-2 rounded-lg border border-[#1b3549] bg-[#091725] px-3 py-2.5 text-[11px] text-[#a9bfcc]"><CheckCircle2 size={14} className="text-[#52d4b0]"/>{permission}</div>)}</div>
        </div>
      </div>
...
function EmployeeCalendarPanel({ employee, day, setDay }: { employee: Employee; day: string; setDay: (value: string) => void }) {
  const events = [
    ...
    ['Tomorrow', employee.tasks[1], '11:00', '#52d4b0'],
    ['Friday', employee.tasks[2], '14:00', '#f6c76d'],
  ];
  return <div className="grid gap-5 lg:grid-cols-[.8fr_1.2fr]">
    <div className="panel p-5"><div className="kicker mb-1">Calendar</div><div className="text-sm font-semibold text-[#deedf1]">Upcoming schedule</div><div className="mt-5 grid grid-cols-3 gap-2">{['Today', 'Tomorrow', 'Friday'].map((item) => <button key={item} onClick={() => setDay(item)} className={`rounded-lg border px-2 py-3 text-[11px] ${day === item ? 'border-[#438fa1] bg-[#153743] text-[#69dce8]' : 'border-[#203a4e] text-[#7892a5]'}`}>{item}</button>)}</div><div className="mt-5 rounded-lg border border-[#203c50] bg-[#0b1928] p-4"><div className="text-[11px] text-[#7892a5]">Next focus</div><div className="mt-2 text-sm font-medium text-[#dcebf0]">{events.find((event) => event[0] === day)?.[1]}</div><div className="mt-2 text-[11px] text-[#58d5e6]">{events.find((event) => event[0] === day)?.[2]} · {day}</div></div></div>
    <div className="panel p-5"><div className="mb-4 flex items-center justify-between"><div><div className="kicker mb-1">Scheduled work</div><div className="text-sm font-semibold text-[#deedf1]">{employee.role} calendar</div></div><CalendarDays size={16} className="text-[#668ba0]"/></div><div className="space-y-3">{events.map(([eventDay, label, time, eventColor]) => <button key={eventDay} onClick={() => { setDay(eventDay); toast.info(`${label} opened for review`); }} className={`flex w-full items-center gap-3 rounded-lg border p-3 text-left ${day === eventDay ? 'border-[#315b6c] bg-[#102b38]' : 'border-transparent bg-[#0b1928]'}`}><span className="h-8 w-1 rounded-full" style={{ background: eventColor }} /><span className="flex-1"><span className="block text-[12px] text-[#d5e5eb]">{label}</span><span className="mt-1 block text-[10px] text-[#718b9f]">{eventDay} · {time}</span></span><ChevronRight size={14} className="text-[#55748b]"/></button>)}</div></div>
  </div>;
}
...
function EmployeeChatPanel({ employee, message, setMessage, sent, send, notifications, setNotifications }: { employee: Employee; message: string; setMessage: (value: string) => void; sent: string[]; send: () => void; notifications: boolean; setNotifications: (value: boolean) => void }) {
  return <div className="panel mx-auto flex min-h-[420px] max-w-[900px] flex-col p-5 md:p-6"><div className="mb-5 flex items-center justify-between"><div><div className="kicker mb-1">AI chat</div><div className="flex items-center gap-2 text-sm font-semibold text-[#deedf1]"><span className="grid h-7 w-7 place-items-center rounded-md text-[10px] font-bold" style={{ background: `${employee.accent}25`, color: employee.color }}>{employee.avatar || employee.initials}</span> Talk to {employee.name}</div></div><button onClick={() => { setNotifications(!notifications); toast.info(`Notifications ${notifications ? 'muted' : 'enabled'}`); }} className={`rounded-md p-2 ${notifications ? 'text-[#58d5e6]' : 'text-[#657e93]'}`} aria-label="Toggle notifications" data-testid="button-toggle-chat-notifications"><Bell size={15}/></button></div><div className="flex-1 space-y-3 text-[12px] leading-5 text-[#8da5b4]"><p className="max-w-[80%] rounded-lg rounded-tl-none bg-[#122738] p-3">I’m ready to help with {employee.role.toLowerCase()} work. What should I review?</p>{sent.map((item, index) => <div key={`${item}-${index}`}><p className="ml-auto max-w-[80%] rounded-lg rounded-tr-none bg-[#183947] p-3 text-[#c3dce2]">{item}</p><p className="mt-2 max-w-[80%] rounded-lg rounded-tl-none bg-[#122738] p-3">I’ll review that using my assigned context and report back with evidence.</p></div>)}</div><div className="mt-5 flex gap-2"><input value={message} onChange={(event) => setMessage(event.target.value)} onKeyDown={(event) => event.key === 'Enter' && send()} className="input-dark h-10 min-w-0 flex-1 rounded-lg px-3 text-xs" placeholder={`Ask ${employee.name} about an HR task...`} data-testid={`input-chat-${employee.id}`}/><button onClick={send} className="btn-primary grid h-10 w-10 place-items-center rounded-lg" data-testid={`button-chat-send-${employee.id}`}><Send size={14}/></button></div></div>;
}

function EmployeeAnalyticsPanel({ employee }: { employee: Employee }) {
  const metrics = [['Task completion', `${Math.max(employee.performance - 2, 0)}%`, '#58d5e6'], ['Response quality', `${employee.performance}%`, employee.color], ['Human review rate', '6.4%', '#f6c76d']];
  return <div className="space-y-5"><div className="grid gap-4 md:grid-cols-3">{metrics.map(([label, value, metricColor]) => <div key={label} className="panel p-5"><div className="kicker">{label}</div><div className="mt-3 display-font text-2xl font-semibold" style={{ color: metricColor }}>{value}</div><div className="mt-1 text-[10px] text-[#71899f]">vs previous period <span className="text-[#52d4b0]">+8.2%</span></div></div>)}</div><div className="panel p-5"><div className="mb-5 flex items-center justify-between"><div><div className="kicker mb-1">Analytics</div><div className="text-sm font-semibold text-[#deedf1]">{employee.name} operating trend</div></div><BarChart3 size={16} style={{ color: employee.color }}/></div><div className="h-36"><Sparkline values={[38, 44, 41, 57, 53, 68, 63, 76, 70, 84, 79, employee.performance]} color={employee.color} fill/></div><div className="mt-3 flex justify-between text-[10px] text-[#687f92]"><span>30 days ago</span><span>Today</span></div></div></div>;
}
...
function EmployeeSettingsPanel({ employee, notifications, setNotifications }: { employee: Employee; notifications: boolean; setNotifications: (value: boolean) => void }) {
  return <div className="panel p-5 md:p-6"><div className="kicker mb-1">Settings</div><div className="mb-5 text-sm font-semibold text-[#deedf1]">Operating configuration</div><div className="grid gap-4 sm:grid-cols-2"><div className="rounded-lg bg-[#0b1928] p-4"><div className="kicker">Manager</div><div className="mt-2 text-sm text-[#d4e5eb]">{employee.manager}</div></div><div className="rounded-lg bg-[#0b1928] p-4"><div className="kicker">Personality</div><div className="mt-2 text-sm text-[#d4e5eb]">{employee.personality || 'Thoughtful and clear'}</div></div></div><div className="mt-4 rounded-lg border border-[#203c50] bg-[#0b1928] p-4"><div className="flex items-center justify-between"><div><div className="text-[12px] font-medium text-[#d3e3e9]">Activity notifications</div><div className="mt-1 text-[11px] text-[#718b9e]">Receive updates when {employee.name} completes work or needs review.</div></div><button onClick={() => { setNotifications(!notifications); toast.info(`Notifications ${notifications ? 'muted' : 'enabled'}`); }} className={`relative h-6 w-11 shrink-0 rounded-full ${notifications ? 'bg-[#3c9fae]' : 'bg-[#263d50]'}`} data-testid="switch-employee-notifications"><span className={`absolute top-1 h-4 w-4 rounded-full bg-[#ecf8f8] ${notifications ? 'left-6' : 'left-1'}`}/></button></div></div><div className="mt-4 rounded-lg border border-[#203c50] bg-[#10283a] p-4"><div className="kicker mb-2">System prompt</div><p className="text-[11px] leading-5 text-[#8da5b4]">{employee.systemPrompt || 'This employee follows the workspace operating context and assigned permissions.'}</p></div></div>;
}
...
function SettingsV2() {
  ...
  return <div className="mx-auto max-w-[980px]"><SectionHeader eyebrow="Workspace / preferences" title="Settings" description="Shape how Orbit Digital works for your team. These preferences persist in this workspace."/><div className="grid gap-5 md:grid-cols-[210px_1fr]"><div className="flex gap-1 overflow-x-auto md:block">{(['Workspace', 'Notifications', 'Security'] as const).map((item) => <button key={item} onClick={() => setSection(item)} className={`mb-1 block whitespace-nowrap rounded-lg px-3 py-2 text-left text-[12px] ${section === item ? 'bg-[#153743] text-[#69dce8]' : 'text-[#7892a5] hover:bg-[#12283a]'}`} data-testid={`button-settings-${item.toLowerCase()}`}>{item}</button>)}</div><div className="space-y-5">
    ...
    {section === 'Security' && <div className="panel p-5 md:p-6"><div className="kicker mb-1">Security</div><div className="mb-5 text-sm font-semibold text-[#e2eff2]">Session and access controls</div><div className="space-y-3 text-[12px]"><div className="flex items-center justify-between rounded-lg bg-[#0b1928] p-4"><span className="text-[#b7cbd4]">Current session</span><span className="status-pill border-[#2f8c7b] bg-[#12352f] text-[#6fe0bd]">Active</span></div><div className="flex items-center justify-between rounded-lg bg-[#0b1928] p-4"><span className="text-[#b7cbd4]">Automatic session expiry</span><span className="text-[#7892a5]">8 hours</span></div><div className="flex items-center justify-between rounded-lg bg-[#0b1928] p-4"><span className="text-[#b7cbd4]">Workspace role</span><span className="text-[#58d5e6]">{user.role}</span></div></div></div>}
...
function RecordModal({ kind, record, onClose }: { kind: 'transactions' | 'customers' | 'merchants'; record: TransactionRecord | CustomerRecord | MerchantRecord; onClose: () => void }) {
  ...
  return <Modal title={`${kind === 'transactions' ? 'Transaction' : kind === 'customers' ? 'Customer' : 'Merchant'} details`} onClose={onClose}>
    <div className="space-y-4">
      ...
      {transaction && <div className="grid grid-cols-2 gap-3 text-[11px]"><div className="rounded-lg bg-[#0b1928] p-3"><div className="kicker">Amount</div><div className="mt-1 text-lg font-semibold text-[#e7f3f5]">${transaction.amount.toLocaleString(undefined, { minimumFractionDigits: 2 })}</div></div><div className="rounded-lg bg-[#0b1928] p-3"><div className="kicker">Payment method</div><div className="mt-2 text-[#b9ccd5]">{transaction.method}</div></div></div>}
      {merchant && <div className="grid grid-cols-2 gap-3 text-[11px]"><div className="rounded-lg bg-[#0b1928] p-3"><div className="kicker">Volume</div><div className="mt-1 text-lg font-semibold text-[#e7f3f5]">${(merchant.volume / 1000000).toFixed(2)}M</div></div><div className="rounded-lg bg-[#0b1928] p-3"><div className="kicker">Auth rate</div><div className="mt-1 text-lg font-semibold text-[#e7f3f5]">{merchant.authRate}%</div></div></div>}
...
function NotificationsPage() {
  ...
  return <div className="mx-auto max-w-[900px]"><SectionHeader eyebrow={`Workspace / ${unreadNotifications} unread`} title="Notifications center." description="Review signals, operational updates, and recommendations from your FinOS team." action={<button onClick={markAllNotificationsRead} className="btn-quiet flex items-center gap-2 rounded-lg px-3 py-2 text-xs" data-testid="button-mark-all-read"><CheckCheck size={14}/> Mark all read</button>}/><div className="panel divide-y divide-[#1a3346]">{notifications.map((item) => <button key={item.id} onClick={() => markNotificationRead(item.id)} className={`flex w-full gap-4 p-5 text-left hover:bg-[#10283a] ${item.read ? 'opacity-70' : ''}`} data-testid={`notification-${item.id}`}><div className={`mt-1 grid h-8 w-8 shrink-0 place-items-center rounded-lg ${item.kind === 'signal' ? 'bg-[#3c2028] text-[#ff897a]' : item.kind === 'success' ? 'bg-[#12352f] text-[#6fe0bd]' : 'bg-[#173746] text-[#58d5e6]'}`}>{item.kind === 'signal' ? <CircleAlert size={15}/> : item.kind === 'success' ? <Check size={15}/> : <Bell size={15}/>}</div><div className="min-w-0 flex-1"><div className="flex flex-wrap items-center justify-between gap-2"><span className="font-medium text-[#dcebf0]">{item.title}</span><span className="text-[10px] text-[#70899d]">{item.time}</span></div><p className="mt-1 text-[12px] leading-5 text-[#8098a8]">{item.detail}</p></div>{!item.read && <span className="mt-2 h-2 w-2 shrink-0 rounded-full bg-[#58d5e6]"/>}</button>)}</div></div>;
}
