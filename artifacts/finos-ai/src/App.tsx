...
function tenantForBackendUser(user: BackendSessionUser) {
  ...
  return { id: user.organization_id, name, domain, initials: initials || 'FN' };
}
...
const employeeAvatarUrl = (employee: Pick<Employee, 'id' | 'name' | 'avatar' | 'role'>) => {
  if (employee.id === 'ceo' || /chief executive officer/i.test(employee.role || '')) return '/atlas-chief-avatar.png';
...
const accountAvatarUrl = (user: { name: string; avatar?: string }) => {
  ...
  return /^https?:\/\//i.test(custom) || custom.startsWith('/')
    ? custom
    : `https://api.dicebear.com/9.x/personas/svg?seed=${encodeURIComponent(user.name)}&backgroundType=gradientLinear&radius=50`;
};
...
function getNavGroups(showMerchant: boolean) {
  return [
    ...
    { label:'Platform', items:[...(showMerchant ? [['/platform-admin', 'Platform admin', ShieldCheck]] : []), ['/integrations', 'Integrations', Zap], ['/email', 'Email center', Send]] },
  ];
}
  ...
  return <div className="noise app-shell flex">
    <aside className={`fixed inset-y-0 left-0 z-40 flex w-[248px] flex-col border-r border-[#172c40] bg-[#090b18] transition-transform duration-200 md:translate-x-0 ${mobileOpen?'translate-x-0':'-translate-x-full'}`}>
      ...
      <div className="border-b border-[#172c40] px-4 py-4">
        <Link href="/profile" className="flex items-center gap-3 rounded-xl px-2 py-2 hover:bg-[#142a3b]" title="Open account profile">
          <img src={accountAvatarUrl(platform.user)} alt={`${platform.user.name} profile`} className="h-10 w-10 rounded-full object-cover ring-2 ring-[#29465d]" referrerPolicy="no-referrer" />
          <div className="min-w-0"><div className="truncate text-[13px] font-semibold text-[#e2edf5]">{platform.user.name}</div><div className="truncate text-[11px] text-[#738ba1]">{platform.user.title || platform.user.role}</div></div>
        </Link>
      </div>
...
function Dashboard() {
  ...
  const chartTransactions = useMemo(() => tenantTransactions.slice().reverse().slice(-12), [tenantTransactions]);
  const chartValues = useMemo(() => {
    const source = chartTransactions.map((tx) => Number(tx.amount || 0));
    if (!source.length) return [];
    const max = Math.max(...source, 1);
    return source.map((value) => Math.max(12, Math.round((value / max) * 88)));
  }, [chartTransactions]);
  const chartVolume = useMemo(() => chartTransactions.reduce((sum, tx) => sum + Number(tx.amount || 0), 0), [chartTransactions]);
      ...
      <div className="panel relative overflow-hidden p-5 md:p-6">
        ...
        <div className="relative mb-6 flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="relative h-[245px]">
            ...
            <div className="absolute inset-0 flex flex-col justify-between text-[10px] text-slate-600"><span>{formatDashboardMoney(chartVolume)}</span><span>75%</span><span>50%</span><span>25%</span><span>$0</span></div>
        ...
        <div className="mt-8 flex flex-wrap items-center gap-5 text-[10px] text-slate-500"><span className="flex items-center gap-2"><i className="h-2 w-2 rounded-full bg-violet-400"/>Payment volume</span><span className="flex items-center gap-2"><i className="h-2 w-2 rounded-full bg-emerald-400"/>Captured: {metrics.captured}</span><span className="ml-auto mono text-slate-300">{range} • {formatDashboardMoney(chartVolume)}</span></div>
      </div>
  ...
  return <Modal title={employee ? `Edit ${employee.name}` : 'Add AI employee'} onClose={onClose}>
    <div className="scrollbar max-h-[78vh] space-y-4 overflow-y-auto pr-1">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <label><span className="kicker mb-2 block">Avatar</span><input value={draft.avatar} onChange={(e) => setDraft({ ...draft, avatar: e.target.value })} className="input-dark h-10 w-full rounded-lg px-3 text-sm" placeholder="Avatar image URL (optional)" data-testid="input-employee-avatar"/></label>
  ...
  return <div className="mx-auto max-w-[1200px]">
    <div className="mb-7 flex flex-col justify-between gap-5 sm:flex-row sm:items-start">
      <div className="flex min-w-0 items-start gap-4">
        <EmployeeAvatar employee={liveEmployee} className="h-16 w-16 rounded-2xl" fallbackClassName="h-16 w-16 bg-[#163147] text-base"/>
...
function AIWorkspace({ employee }: { employee:Employee }) {
  ...
  const send=async()=>{
    ...
    try {
      ...
      setResponses((current)=>[...current,`I’m currently unable to reach my AI service: ${detail}`]);
      toast.error(`${employee.name} could not reach the AI backend`);
...
function IntegrationsPage() {
  const providers = [
    { name: 'Fawry', kind: 'Payment gateway', status: 'Ready for secure server credentials', env: 'FAWRY_MERCHANT_CODE / FAWRY_SECRET' },
    { name: 'Aman', kind: 'Payment gateway', status: 'Ready for secure server credentials', env: 'AMAN_MERCHANT_ID / AMAN_SECRET' },
    { name: 'Email / SMTP', kind: 'Operational messaging', status: 'Use server-side provider credentials', env: 'EMAIL_PROVIDER / EMAIL_API_KEY' },
    { name: 'Webhooks', kind: 'Company event integration', status: 'Endpoint configuration ready', env: 'INTEGRATION_WEBHOOK_SECRET' },
  ];
  return <div className="mx-auto max-w-[1180px]">
    <SectionHeader eyebrow="Platform / integrations" title="Company integrations." description="Payment, messaging, and webhook connections are kept out of the browser. Store real secrets only in server-side environment variables or a secret manager." />
    <div className="mb-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{providers.map((provider) => <div key={provider.name} className="panel p-5"><div className="mb-4 flex items-center justify-between"><div className="grid h-9 w-9 place-items-center rounded-lg bg-[#173b48] text-[#8b5cf6]"><Zap size={16}/></div><span className="status-pill border-[#167d5a] bg-[#0d3b2c] text-[#6fe0bd]">Configured UI</span></div><div className="text-sm font-semibold text-[#e2e8f0]">{provider.name}</div><div className="mt-1 text-[10px] text-[#71899d]">{provider.kind}</div><div className="mt-4 text-[11px] leading-5 text-[#9bb1bd]">{provider.status}</div><div className="mt-3 rounded-lg bg-[#0b1020] p-3"><div className="kicker mb-1">Server secret names</div><div className="mono break-words text-[9px] text-[#7f9aaa]">{provider.env}</div></div></div>)}</div>
    <div className="grid gap-5 lg:grid-cols-2">
      <div className="panel p-5 md:p-6"><div className="kicker mb-2">Online operations</div><div className="text-sm font-semibold text-[#e2e8f0]">Customer-service visibility</div><p className="mt-2 text-[11px] leading-5 text-[#7892a5]">Transactions, customer records, risk signals, and platform activity are already surfaced in the dashboard. The payment providers above must be wired through the API server before live money movement is enabled.</p><div className="mt-5 grid gap-2 sm:grid-cols-3">{[['Payments', '/transactions'], ['Customers', '/customers'], ['Platform activity', '/platform-admin']].map(([label, path]) => <Link key={path} href={path} className="btn-quiet rounded-lg px-3 py-3 text-center text-[11px]">{label}</Link>)}</div></div>
      <div className="panel p-5 md:p-6"><div className="kicker mb-2">Security boundary</div><div className="text-sm font-semibold text-[#e2e8f0]">No credentials in the UI</div><div className="mt-4 space-y-2 text-[11px] text-[#91a8b5]"><div className="rounded-lg bg-[#0c1020] p-3">✓ API keys stay server-side.</div><div className="rounded-lg bg-[#0c1020] p-3">✓ Frontend stores no provider secret.</div><div className="rounded-lg bg-[#0c1020] p-3">✓ Signed/private knowledge-file access remains server-controlled.</div></div></div>
    </div>
  </div>;
}
...
function EmailCenterPage() {
  ...
  const employee = roster.find((item) => item.id === employeeId) || roster[0];
  const send = () => {
    if (!to.trim() || !subject.trim() || !body.trim()) { toast.error('Add recipient, subject, and message'); return; }
    const params = new URLSearchParams({ subject: subject.trim(), body: `${body.trim()}\n\nSent from FinOS by ${employee?.name || user.name}.` });
    window.location.href = `mailto:${to.trim()}?${params.toString()}`;
    toast.success('Email draft opened in the device email client');
  };
  return <div className="mx-auto max-w-[900px]"><SectionHeader eyebrow="Communications / email" title="Send email safely." description="Employees can prepare messages without seeing any Gmail, Outlook, SMTP, or API credentials. The browser opens the user's configured mail client; live provider sending should be connected server-side."/><div className="panel p-5 md:p-6"><div className="grid gap-4 sm:grid-cols-2"><label><span className="kicker mb-2 block">Sending employee</span><select value={employeeId} onChange={(event) => setEmployeeId(event.target.value)} className="input-dark h-10 w-full rounded-lg px-3 text-sm">{roster.map((item) => <option key={item.id} value={item.id}>{item.name} • {item.role}</option>)}</select></label><label><span className="kicker mb-2 block">Recipient</span><input type="email" value={to} onChange={(event) => setTo(event.target.value)} className="input-dark h-10 w-full rounded-lg px-3 text-sm" placeholder="recipient@company.com"/></label></div><label className="mt-4 block"><span className="kicker mb-2 block">Subject</span><input value={subject} onChange={(event) => setSubject(event.target.value)} className="input-dark h-10 w-full rounded-lg px-3 text-sm" placeholder="FinOS operational update"/></label><label className="mt-4 block"><span className="kicker mb-2 block">Message</span><textarea value={body} onChange={(event) => setBody(event.target.value)} className="input-dark min-h-[220px] w-full resize-y rounded-lg px-3 py-3 text-sm leading-6" placeholder="Write the message..."/></label><div className="mt-4 flex flex-wrap items-center justify-between gap-3"><div className="text-[10px] text-[#71899d]">Credentials are never displayed or stored here.</div><button onClick={send} className="btn-primary flex items-center gap-2 rounded-lg px-4 py-2.5 text-xs"><Send size={14}/> Open email client</button></div></div></div>;
}
...
function Login({ onLogin }: { onLogin: () => void }) {
  ...
  const beginOnboarding = (type: 'company' | 'individual') => {
    setError('');
    setAccountType(type);
    setSubscription(type === 'company' ? 'company_15' : 'merchant_basic');
    setOnboarding(true);
    setStep(1);
    setPassword('');
    setConfirmPassword('');
  ...
  const nextOnboardingStep = () => {
    setError('');
    if (step === 1 && (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(normalizeEmail(email)) || password.length < 8 || password !== confirmPassword || !fullName.trim() || !/^\+?[0-9\s-]{8,15}$/.test(phone.trim()) || !/^\d{14}$/.test(idNumber.trim()))) {
      setError('Enter your name, a valid email, a valid phone number, a 14-digit national ID number, and matching passwords of at least 8 characters.');
      return;
    }
  ...
  const onboardingPanel = (
    <>
      ...
      {step === 1 && <div className="space-y-4">
        ...
        <label className="block"><span className="kicker mb-2 block">National ID number</span><input value={idNumber} onChange={(event) => setIdNumber(event.target.value.replace(/\D/g, '').slice(0, 14))} inputMode="numeric" maxLength={14} className={fieldClass} placeholder="14-digit national ID" data-testid="input-signup-id-number"/></label>
        <div className="grid gap-2 sm:grid-cols-2"><button onClick={() => { setAccountType('company'); setSubscription('company_15'); }} className={`rounded-lg border px-3 py-3 text-left text-[11px] ${accountType === 'company' ? 'border-[#8b5cf6] bg-[#21183f] text-white' : 'border-[#24465c] text-[#7892a5]'}`}><b>Company</b><span className="mt-1 block">Subscription workspace</span></button><button onClick={() => { setAccountType('individual'); setSubscription('merchant_basic'); }} className={`rounded-lg border px-3 py-3 text-left text-[11px] ${accountType === 'individual' ? 'border-[#8b5cf6] bg-[#21183f] text-white' : 'border-[#24465c] text-[#7892a5]'}`}><b>Individual / Merchant</b><span className="mt-1 block">Merchant / small business</span></button></div>
      </div>}
      ...
      {step === 2 && accountType === 'individual' && <div className="space-y-4">
        ...
        <label><span className="kicker mb-2 block">Subscription</span><select value={subscription} onChange={(event) => setSubscription(event.target.value as 'merchant_basic' | 'merchant_premium' | 'company_15' | 'company_32')} className={fieldClass} data-testid="select-individual-subscription"><option value="merchant_basic">Basic — merchant workspace — $100/mo</option><option value="merchant_premium">Premium — merchant workspace — $200/mo</option></select></label>
      ...
      <div className="relative hidden w-[48%] overflow-hidden border-r border-[#193149] lg:block">
        ...
        <div className="relative flex h-full flex-col justify-between p-12">
          ...
          <div className="max-w-[420px]">
            ...
            <p className="mt-7 max-w-[360px] text-[14px] leading-6 text-[#8299ad]">FinOS gives your finance, risk, and merchant teams a shared view of money in motion — with an AI team that knows what to do next.</p>
          </div>
          ...
          {onboarding ? onboardingPanel : (
            <>
              ...
              <div className="mt-8 space-y-4">
                ...
                <p className="mt-6 text-center text-[10px] leading-5 text-[#637e93]">Every workspace requires a registered email and password.</p>
              </div>
            </>
          )}
        </div>
...
function PlatformAdminPage() {
  ...
  const upgradeToPremium = async (organizationId: string, organizationName: string) => {
    if (!window.confirm(`هل تريد ترقية اشتراك ${organizationName} إلى Premium؟`)) return;
    ...
    try {
      ...
      toast.success(`تم تحويل اشتراك ${organizationName} إلى Premium بنجاح`);
  ...
  const changeOrganizationStatus = async (organizationId: string, organizationName: string, currentStatus: string) => {
    const nextStatus = currentStatus.toLowerCase() === 'active' ? 'suspended' : 'active';
    const label = nextStatus === 'active' ? 'تشغيل' : 'إيقاف';
    if (!window.confirm(`هل تريد ${label} حساب ${organizationName}؟`)) return;
    setUpgradingOrganizationId(organizationId);
    try {
      const response = await fetch(`/api/platform-admin/organizations/${encodeURIComponent(organizationId)}/status`, {
        method: 'POST',
        headers: { 'x-finos-platform-admin-email': adminEmail, Accept: 'application/json', 'Content-Type': 'application/json' },
        body: JSON.stringify({ status: nextStatus }),
      });
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(typeof payload?.error === 'string' ? payload.error : 'Unable to change organization status.');
      toast.success(nextStatus === 'active' ? 'تم تشغيل الحساب بنجاح' : 'تم إيقاف الحساب بنجاح');
      await analyticsQuery.refetch();
    } catch (error) {
      toast.error(error instanceof Error ? error.message : 'Unable to change organization status.');
    } finally {
      setUpgradingOrganizationId(null);
    }
  };

  const resetUserPassword = async (organizationId: string, organizationName: string) => {
    const targetEmail = window.prompt(`اكتب بريد الحساب داخل ${organizationName} الذي تريد تغيير كلمة مروره:`)?.trim().toLowerCase();
    if (!targetEmail) return;
    const temporaryPassword = `FinOS-${crypto.randomUUID().replace(/-/g, '').slice(0, 16)}!`;
    if (!window.confirm(`سيتم تعيين كلمة مرور مؤقتة للحساب ${targetEmail} وإبطال جلساته الحالية.\n\nسيتم عرض كلمة المرور بعد النجاح. هل تريد المتابعة؟`)) return;
    setUpgradingOrganizationId(organizationId);
    try {
      const response = await fetch('/api/platform-admin/users/password', {
        method: 'POST',
        credentials: 'include',
        headers: { 'x-finos-platform-admin-email': adminEmail, Accept: 'application/json', 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: targetEmail, temporary_password: temporaryPassword }),
      });
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(typeof payload?.error === 'string' ? payload.error : 'Unable to change this password.');
      window.alert(`تم تغيير كلمة المرور بنجاح.\n\nالحساب: ${targetEmail}\nكلمة المرور المؤقتة: ${temporaryPassword}\n\nيجب تسليمها لصاحب الحساب عبر قناة آمنة، ثم تغييرها بعد الدخول.`);
      toast.success(`تم تغيير كلمة مرور ${targetEmail} وإبطال الجلسات السابقة`);
    } catch (error) {
      toast.error(error instanceof Error ? error.message : 'Unable to change this password.');
    } finally {
      setUpgradingOrganizationId(null);
    }
  };
  ...
  return <div className="mx-auto max-w-[1450px]">
    ...
    <div className="mb-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      ...
      <MetricCard label="Monthly expected revenue" value={formatPlatformCurrency(summary.monthly_expected_revenue_cents)} delta={`${summary.basic_subscriptions} Basic • ${summary.premium_subscriptions} Premium`} icon={CircleDollarSign} color="#34d399"/>
    ...
    {recentActivity.some((event) => event.event_type === 'employee_complaint') && <div className="panel mb-6 overflow-hidden border border-[#7b4f43]"><div className="flex items-center gap-3 border-b border-[#5a3a38] bg-[#2a1b24] px-5 py-4 md:px-6"><div className="grid h-9 w-9 place-items-center rounded-full bg-[#6d3840] text-[#ffb4aa]"><CircleAlert size={17}/></div><div><div className="text-sm font-semibold text-[#ffe1dc]">Employee complaint alert</div><div className="mt-1 text-[11px] text-[#d9a8a2]">The chief assistant received a complaint about an employee. Review the recent event below and follow up with the workspace.</div></div></div><div className="space-y-2 p-5">{recentActivity.filter((event) => event.event_type === 'employee_complaint').slice(0, 3).map((event) => <div key={event.id} className="rounded-lg border border-[#5a3a38] bg-[#211820] p-3 text-[11px] text-[#e8c1bb]"><div className="font-semibold">New complaint received</div><div className="mt-1 text-[#bc8d8a]">{new Intl.DateTimeFormat(undefined, { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' }).format(new Date(event.created_at))}</div></div>)}</div></div>}
